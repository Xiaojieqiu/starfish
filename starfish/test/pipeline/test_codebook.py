...
def test_euclidean_decode_yields_correct_output(euclidean_decoded_intensities):
    ...
    expected_qualities = np.array([1, 1, 1, 0.5, 1])
    observed_qualities = euclidean_decoded_intensities[
        IntensityTable.Constants.QUALITY.value].values
    assert np.array_equal(expected_gene_annotation, observed_gene_annotation)
    assert np.array_equal(expected_qualities, observed_qualities)
